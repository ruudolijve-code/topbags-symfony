...
final class StyleGuideAffinityCrudController extends AbstractCrudController
{
    ...
    {
        return StyleGuideAffinity::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('Stijlaffiniteit')
            ->setEntityLabelInPlural('Stijlaffiniteiten')
            ->setDefaultSort([
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        yield IntegerField::new('score', 'Score')
            ->setHelp(
                'Sterke positieve affiniteiten liggen doorgaans tussen +15 en +40.'
            );

        yield TextField::new('reason', 'Reden')
            ->setRequired(false)
            ->setHelp(
                'Wordt als matchreden aan de klant getoond bij een positieve match.'
            );
